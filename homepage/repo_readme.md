![Version](https://img.shields.io/github/v/release/DCMLab/debussy_suite_bergamasque?display_name=tag)
...
<!-- TOC -->
* [Claude Debussy - Suite Bergamasque (A corpus of annotated scores)](#claude-debussy---suite-bergamasque--a-corpus-of-annotated-scores-)
...
the first movement (Prélude of item 75 in the Lesure (1977) catalogue) has the following files:

* `MS3/l075-01_suite_prelude.mscx`: Uncompressed MuseScore file including the music and annotation labels.
* `notes/l075-01_suite_prelude.tsv`: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
* `measures/l075-01_suite_prelude.tsv`: A table with relevant information about the measures in the score.
* `harmonies/l075-01_suite_prelude.tsv`: A list of the included harmony labels (including cadences and phrases) with their positions in
  the score.
...
labels = ms3.load_tsv('harmonies/l075-01_suite_prelude.tsv')
notes = ms3.load_tsv('notes/l075-01_suite_prelude.tsv')
```
...
[Lesure (1977) catalogue](https://en.wikipedia.org/wiki/List_of_compositions_by_Claude_Debussy) number 75.
